...
export const createTransaction = async (req, res, next) => {
  try {
    ...
    if (wallet) {
        ...
        wallet.balance = parseFloat((wallet.balance + purchasedMinutes).toFixed(2));
    
        // Ensure minutes in wallet activity are also rounded
        wallet.walletActivity.push({
          status: "+",
          minute: parseFloat(purchasedMinutes.toFixed(2)), 
          time: transactionTime
        });
    } else {
        wallet = new Wallet({
          customerId,
          balance: parseFloat(purchasedMinutes.toFixed(2)), 
          walletActivity: [
            {
              status: "+",
              minute: parseFloat(purchasedMinutes.toFixed(2)), 
